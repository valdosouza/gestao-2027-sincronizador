...
interface
uses IBX.IBDatabase,Classes,  SysUtils,ControllerBase,tblRetaguardaWeb ,
      Generics.Collections, IBX.IBQuery,ControllerRetaguardaSync,
      ControllerTrigger;
...
Type
  ...
  private
    ...
    procedure execSQL(Qry:TIBQuery);
  public
...
begin
  ...
  FBancoRetaguarda := TIBDatabase.Create(self);
...
procedure TControllerRetaguardaWeb.execSQL(Qry: TIBQuery);
begin
  with Qry do
  Begin
    try
...
procedure TControllerRetaguardaWeb.setFBancoPDV(const Value: TIBDatabase);
begin
  FBancoPDV := Value;
end;
