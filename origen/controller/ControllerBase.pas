...
uses Classes,SysUtils, DskGenericDao,
     UnFunctions, Windows,Gauges, Un_DM, IBX.IBQuery,
    IBX.IBDatabase, IBX.IBScript;
...
Type
  ...
  protected
    ...
    function getByField(strTable:String;field:String; Content:String):TIBQuery;
    procedure _getByKey<T: class>(Obj: T);
    function existObj<T: class>(Obj: T):Boolean;

    function NomeComputador: string;
    procedure get<T: class>(Qry: TIBQuery;Obj: T);
  public
    exist : Boolean;
    update : Boolean;
    ...
    Function GeraQuery():TIBQuery;
    Function GeraIBScript():TIBScript;
    procedure FinalizaQuery(Qry:TIBQuery);
    procedure FinalizaISQL(Script: TIBScript);
...
procedure TControllerBase.FinalizaISQL(Script: TIBScript);
begin
  try
    if Script.Transaction.InTransaction then Script.Transaction.Commit;
  finally
...
procedure TControllerBase.FinalizaQuery(Qry: TIBQuery);
begin
  try
    IF Qry.Transaction.InTransaction then Qry.Transaction.Commit;
  finally
...
Function TControllerBase.GeraQuery():TIBQuery;
VAr
  Lc_Tsc : TIBTransaction;
begin
  Result := TIBQuery.Create(Self);
  Lc_Tsc := TIBTransaction.Create(nil);
...
function TControllerBase.getByField(strTable, field, Content: String): TIBQuery;
var
  vSelect: string;
begin
  Result := GeraQuery;
...
procedure TControllerBase.setFDataBase(const Value: TIBDatabase);
begin
  FDataBase := Value;
end;
...
procedure TControllerBase.get<T>(Qry: TIBQuery;Obj: T) ;
begin
  clearObj(Obj);
...
function TControllerBase.GeraIBScript: TIBScript;
VAr
  Lc_Tsc : TIBTransaction;
begin
  Result := TIBScript.Create(Self);
  Lc_Tsc := TIBTransaction.Create(nil);
  if not FDataBase.Connected then
    FDataBase.Connected := True;
  Lc_Tsc.DefaultDatabase := FDataBase;
