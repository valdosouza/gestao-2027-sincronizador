...
Var
  ...
  LcAddress : TAddress;
begin
  pObj.Entidade.Registro.Codigo              := 0;//Sempre zero pois ser� redefnido no Sistema Web
  ...
  pObj.Entidade.Registro.RamoAtividade       := pEmpresa.CodigoRamoAtividade;
  pObj.Entidade.Registro.Observacao          := pEmpresa.Observacao;
  ...
  //EntityExternalCode
  pObj.EntityExternalCode.Codigo          := 0;
  pObj.EntityExternalCode.Estabelecimento := pObj.Entidade.Estabelecimento;
  pObj.EntityExternalCode.Referencia      := pEmpresa.Codigo;
  pObj.EntityExternalCode.Tipo            := 'EMPRESA';

  if pEmpresa.TipoPessoa = 'F' then
  Begin
    pObj.Fisica.Codigo              := 0; //Sempre zero pois ser� redefnido no Sistema Web
  ...
  Begin
    pObj.Juridica.Codigo                  := 0; //Sempre zero pois ser� redefnido no Sistema Web
  ...
  Begin
    ...
    pObj.Entidade.setArrayAddress(1);
    pObj.Entidade.ListaEndereco[0] := LcAddress;

    pObj.Entidade.setArrayPhone(0);
    ...
    BEgin
      ...
      pObj.Entidade.setArrayPhone(Length(pObj.Entidade.ListaFones) + 1);
      pObj.Entidade.ListaFones[Length(pObj.Entidade.ListaFones) -1] := LcPhone;
    End;
  ...
  Begin
    pObj.Entidade.Email.Codigo := 0; //Sempre zero pois ser� redefnido no Sistema Web
  ...
  Begin
    pObj.Entidade.MidiaSocial.Codigo := 0; //Sempre zero pois ser� redefnido no Sistema Web
...
begin
  ...
  Begin
    Codigo                            := 0;
    Tipo                              := 1;
    NomeRazaoSocial                   := Fiscal.Entidade.Registro.NomeRazao;
    ApelidoFantasia                   := Fiscal.Entidade.Registro.ApelidoFantasia;
    ...
    End;
    SituacaoCredito                   := 'L';
    Observacao                        := Fiscal.Entidade.Registro.Observacao;
    DataCadastro                      := Fiscal.Entidade.Registro.RegistroCriado;
    ValorCredito                      := 0;
    CodigoVendedor                    := 0;
    ...
    DataFundacao                      := Fiscal.Entidade.Registro.AniversarioFundacao ;
  ...
  if Length(Fiscal.Entidade.ListaEndereco) > 0 then
  BEgin
    ...
    Begin
      ...
      //Telefone
      for I := 0 to Length(Fiscal.Entidade.ListaFones) -1 do
      Begin
  ...
  End;
  Fiscal.Entidade.Registro.Codigo :=  Registro.Codigo;
end;
