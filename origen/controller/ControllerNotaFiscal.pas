...
Type
  ...
  private
    ...
    procedure FillDataInvoice(PcRegistro: TNotaFiscal;ObjInvoice:TObjInvoice);
    procedure FillDataInvoiceMerchandise(PcRegistro: TNotaFiscal);
    procedure FillDataInvoiceShipping(PcRegistro: TNotaFiscal);
    procedure FillDataInvoiceObservaocao(PcRegistro: TNotaFiscal);
  ...
  public
    ...
    ObjMerchandise: TObjInvoiceMerchandise;
    ...
    procedure FillDataObjetoMerchandise(PcRegistro: TNotaFiscal);
    procedure FillDataObjetoDetached(PcRegistro: TNotaFiscal);
    ...
    PROCEDURE AtualizaSeries(Disponivel:String;Lista:TIBQuery);
...
procedure TControllerNotaFiscal.AtualizaSeries(Disponivel:String; Lista: TIBQuery);
Var
  Oper : String;
begin
  Lista.First;
...
Begin
  ...
  Try
    ...
    Begin
      ...
      ParamByName('nfl_ctrl_retorno').AsAnsiString:= AnsiString(Registro.ControlaRetorno);
      ParamByName('NFL_NFL_VINCULO').AsAnsiString := AnsiString(Registro.NotaVinculada);
      ExecSQL;
    end;
  Finally
    FinalizaQuery( Lc_Qry );
...
begin
  ...
  ObjMerchandise := TObjInvoiceMerchandise.Create;
...
procedure TControllerNotaFiscal.FillDataObjetoDetached(PcRegistro: TNotaFiscal);
begin
  //Nota
  FillDataInvoice(PcRegistro,ObjInvoice);
  //Observa��es
  FillDataInvoiceObservaocao(PcRegistro);
end;

procedure TControllerNotaFiscal.FillDataObjetoMerchandise(
  PcRegistro: TNotaFiscal);
begin
  //Nota
  FillDataInvoice(PcRegistro,ObjMerchandise);
  //Entrega
  FillDataInvoiceShipping(PcRegistro);
  //Observa��es
  FillDataInvoiceObservaocao(PcRegistro);
  if PcRegistro.CodigoPedido > 0 then
  Begin
    //NOta Mercadoria
    FillDataInvoiceMerchandise(PcRegistro);
    //ICMS
    ...
    //IMPORTA��O - Obj.ItemsII
    FillDataObjetoNotaItemsII(PcRegistro);
  End;
end;

procedure TControllerNotaFiscal.FillDataInvoiceMerchandise(PcRegistro: TNotaFiscal);
begin
  //Nota de Mercadoria
  ObjMerchandise.NotaMercadoria.Codigo             := ObjMerchandise.Nota.Codigo;
  ObjMerchandise.NotaMercadoria.Estabelecimento    := ObjMerchandise.Estabelecimento;
  ObjMerchandise.NotaMercadoria.Terminal           := ObjMerchandise.Terminal;
  ObjMerchandise.NotaMercadoria.DataSaida          := PcRegistro.DataSaida;
  ObjMerchandise.NotaMercadoria.HoraSaida          := PcRegistro.HoraSaida;
  ObjMerchandise.NotaMercadoria.ValorBaseICMS      := PcRegistro.ValorBaseICMS;
  ObjMerchandise.NotaMercadoria.ValorIcms          := PcRegistro.ValorICMS;
  ObjMerchandise.NotaMercadoria.ValorBaseIcmsSt    := PcRegistro.ValorBaseICMSST;
  ObjMerchandise.NotaMercadoria.ValorIcmsSt        := PcRegistro.ValorICMSST;
  ObjMerchandise.NotaMercadoria.ValorTotal         := PcRegistro.ValorProdutos;
  ObjMerchandise.NotaMercadoria.ValorFrete         := PcRegistro.ValorFrete;
  ObjMerchandise.NotaMercadoria.ValorSeguro        := PcRegistro.ValorSeguro;
  ObjMerchandise.NotaMercadoria.ValorDespesas      := PcRegistro.ValorOutrasDespesas;
  ObjMerchandise.NotaMercadoria.ValorIpi           := PcRegistro.ValorIPI;
  ObjMerchandise.NotaMercadoria.ValorDesc          := pedido.Registro.ValorDesconto;
  ObjMerchandise.NotaMercadoria.QuantidadeTotal    := PcRegistro.QtdeProdutos;
  ObjMerchandise.NotaMercadoria.IndicacaoPresenca  := pedido.Registro.IndicaPresenca;

end;

procedure TControllerNotaFiscal.FillDataInvoiceObservaocao(
  PcRegistro: TNotaFiscal);
Var
  LcObs : TInvoiceObs;
  I : Integer;
begin
  ...
  Observacoes.getList;
  ObjMerchandise.setArrayInvoiceObs(0);
  if (Observacoes.Lista.Count > 0) then
  Begin
    ObjMerchandise.setArrayInvoiceObs(Observacoes.Lista.Count);
    for I := 0 to Observacoes.Lista.Count - 1 do
    Begin
      LcObs := TInvoiceObs.Create;
      LcObs.Codigo          := Observacoes.Lista[I].Codigo;
      LcObs.Estabelecimento := ObjMerchandise.Estabelecimento;
      LcObs.NotaFiscal      := ObjMerchandise.Nota.Codigo;
      LcObs.Terminal        := ObjMerchandise.Nota.Terminal;
      LcObs.Observacao      := Observacoes.Lista[I].Conteudo;
      ObjMerchandise.Observacao[I] := LcObs;
    End;
  End;
end;

procedure TControllerNotaFiscal.FillDataInvoiceShipping(PcRegistro: TNotaFiscal);
begin
  //Nota de Mercadoria  - Entrega
  with ObjMerchandise.Entrega do
  Begin
    Codigo                := ObjMerchandise.Nota.Codigo;
    Estabelecimento       := ObjMerchandise.Estabelecimento;
    Terminal              := ObjMerchandise.Terminal;
    Quantidade            := PcRegistro.QtdeProdutos;
    Classificacao         := PcRegistro.Especie;
    Marca                 := PcRegistro.Marca;
    PesoBruto             := PcRegistro.PesoBruto;
    PesoLiquido           := PcRegistro.PesoLiquido;
    NumeroVolume          := PcRegistro.NumeroVolume;
    PlacaVeiculo          := PcRegistro.PlacaVeiculo;
    PlacaEstado           := PcRegistro.EstadoVeiculo;
    PlacaRntc             := PcRegistro.RNTC;
  End;
end;

procedure TControllerNotaFiscal.FillDataInvoice(PcRegistro: TNotaFiscal;ObjInvoice:TObjInvoice);
begin
  //Order
  if PcRegistro.CodigoPedido > 0 then
  Begin
  ...
  with ObjInvoice do
  Begin
    Nota.Codigo          := PcRegistro.Codigo;
    Nota.Estabelecimento := Estabelecimento;
    Nota.Terminal        := Terminal;
    // IssuerExternalCode/EntityExternalCode removidos: TInvoice (destiny/
    // model) não tem mais essas propriedades (Integer disfarçado de
    // "external code", mesmo padrão já corrigido em Customer/Provider/
    // SalesMan/Order*). invoice_send_web.pas já resolve o documento do
    // destinatário via DM.GetDocumentByEmpCodigo(PcRegistro.CodigoEmpresa)
    // por conta própria — este método não precisa mais fazer esse trabalho.
    Nota.Emitente           := 0;
    Nota.Destinatario       := 0;
    Nota.TipoEmissao        := PcRegistro.Tipo;
    Nota.Finalidade         := PcRegistro.Finalidade;
    Nota.Numero             := PcRegistro.Numero;
    Nota.Serie              := PcRegistro.Serie;

    if (PcRegistro.CFOP <> '') then
      Nota.Cfop               := PcRegistro.CFOP
    else
      Nota.Cfop               := getCFOPByCodigo(PcRegistro.CodigoCFOP);

    Nota.Data_emissao       := PcRegistro.DataEmissao;
    Nota.Valor              := PcRegistro.ValorNota;
    Nota.Modelo             := PcRegistro.Modelo;
    Nota.Status             := PcRegistro.Situacao;
  End;
end;
...
begin
  ...
  ObjMerchandise.setArrayItemsCofins(0);
  if Pedido.Itens.ItensCofins.Lista.Count > 0 then
  Begin
    ObjMerchandise.setArrayItemsCofins(Pedido.Itens.ItensCofins.Lista.Count);
    ...
    Begin
      ...
      LcCfs.Ordem               := ObjMerchandise.Nota.Codigo;
      LcCfs.Estabelecimento     := ObjMerchandise.Estabelecimento;
      LcCfs.Terminal            := ObjMerchandise.Terminal;
      LcCfs.CST                 := '';
      LcCfs.Valor               := Pedido.Itens.ItensCofins.Lista[I].ValorBase;
      ...
      ObjMerchandise.ItemsCofins[I] := LcCfs;
    End;
  End;
end;
...
begin
  ...
  ObjMerchandise.setArrayItemsIcms(0);
  if (Pedido.Itens.ItensIcms.Lista.Count > 0) then
  Begin
    ObjMerchandise.setArrayItemsIcms(Pedido.Itens.ItensIcms.Lista.Count);
    ...
    Begin
      ...
      LcIcms.Ordem              := ObjMerchandise.Nota.Codigo;
      LcIcms.Estabelecimento    := ObjMerchandise.Estabelecimento;
      LcIcms.Terminal           := ObjMerchandise.Terminal;
      ...
      // TOrderItemIcms (destiny/model) tem um typo no próprio nome da
      // propriedade: "DetermincaoBase"/"DetermincaoBaseST" (falta o "a").
      ...
      ObjMerchandise.ItemsICMS[I] := LcIcms;
    End;
  End;
end;
...
begin
  ...
  ObjMerchandise.setArrayItemsII(0);
  if (Pedido.Itens.ItensII.Lista.Count > 0) then
  Begin
    ObjMerchandise.setArrayItemsII(Pedido.Itens.ItensII.Lista.Count);
    ...
    Begin
      ...
      LcII.Ordem                := ObjMerchandise.Nota.Codigo;
      LcII.Estabelecimento      := ObjMerchandise.Estabelecimento;
      LcII.Terminal             := ObjMerchandise.Terminal;
...
begin
  ...
  ObjMerchandise.setArrayItemsIpi(0);
  if (Pedido.Itens.ItensIpi.Lista.Count > 0) then
  Begin
    ObjMerchandise.setArrayItemsIpi(Pedido.Itens.ItensIpi.Lista.Count);
    ...
    Begin
      ...
      LcIPI.Ordem                 := ObjMerchandise.Nota.Codigo;
      LcIPI.Estabelecimento       := ObjMerchandise.Estabelecimento;
      LcIPI.Terminal              := ObjMerchandise.Terminal;
      ...
      // TOrderItemIpi (destiny/model): Base->ValorBase, Aliquota->ValorAliquota,
      // Valor->ValorUnitario (nomes reais das propriedades).
      ...
      ObjMerchandise.ItemsIPI[I]  := LcIPI;
    End;
  End;
end;
...
Begin
  ...
  ObjMerchandise.setArrayItemsPis(0);
  if (Pedido.Itens.ItensPis.Lista.Count > 0) then
  Begin
    ObjMerchandise.setArrayItemsPis(Pedido.Itens.ItensPis.Lista.Count);
    ...
    Begin
      ...
      LcPis.Ordem                 := ObjMerchandise.Nota.Codigo;
      LcPis.Estabelecimento       := ObjMerchandise.Estabelecimento;
      LcPis.Terminal              := ObjMerchandise.Terminal;
      LcPis.CST                   := getCST_PISByCodigo(Pedido.Itens.ItensPis.Lista[I].CST);
      // TOrderItemPis (destiny/model): Base->ValorBase (Aliquota/Valor já batem).
      ...
      ObjMerchandise.ItemsPIS[I]  := LcPis;
    End;
  End;
end;
...
begin
  ...
  //Lc_Subtotal := Lc_Subtotal + registro.ValorServico; j� est� no pedido
