...
interface
uses IBDatabase,Classes, IBQuery, SysUtils,ControllerBase,Vcl.Forms,
...
Type
  ...
  public
    ...
    procedure MigraSelf(Lc_Qry: TIBQuery);
    ...
    BEgin
     ...
     //Objeto Mercadoria
      ...
      BEgin
        ...
        Origem            := Prod.Origem;
        Tipo              := Prod.Tipo;
        // Marca/name_brand: TMerchandise (destiny/model) não tem propriedade
        // name_brand (não é coluna de tb_merchandise) — a resolução da
        // descrição da marca por documento foi movida para
        // classes/merchandise_send_web.pas, que já monta o payload direto.
        Marca             := 0;
      ...
      BEgin
        ...
        Estabelecimento   := institutioWebId;
        // Embalagem/Medida/Cor: mesma ressalva da Marca acima — TStock
        // (destiny/model) não tem name_package/name_measure/name_color;
        // resolução por descrição feita em merchandise_send_web.pas.
        Embalagem         := 0;
        Medida            := 0;
        Cor               := 0;
...
procedure TControllerProduto.migraSelf(Lc_Qry: TIBQuery);
begin
  get(Lc_qry,Registro);
end;
...
begin
  Try
    ...
    Begin
      ...
      End;
      //Agora inicia do 1 para ajustar a orde�a��o
      getList('');
      FProgresso.Progress := 0;
      FProgresso.MinValue := 0;
      FProgresso.MaxValue := Lista.Count;
...
begin
  ...
  Begin
    ...
    // TODO (D16 - sentido inverso web->Firebird, fase própria ainda não
    // iniciada): TMerchandise/TStock/TProduct (destiny/model) não carregam
    // mais name_brand/name_measure/name_package/name_category — eram
    // campos transitórios só de exibição, removidos do DTO. A resolução
    // "autocreate por descrição" precisa ser redesenhada quando a Fase 2
    // de recebimento for planejada (provavelmente via CONTRATOS_SYNC novo,
    // com descrição vindo em campo próprio do payload de entrada). Por ora
    // mantém os códigos como já estavam no registro local (não sobrescreve).
...
begin
  Try
    ...
    Begin
      ...
      //Criei objeto contorller para produto e para menu cardapio  aqui � diferente de automatico -
