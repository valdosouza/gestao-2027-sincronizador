...
uses
   ...
   ControllerListaSincronia, tblEmpresa, tblSincronia;
...
type
  ...
  TControllerRetaguardaSendToLocal = Class(TComponent)
    ...
    FDBRetaguarda: TIBDatabase;
    FDBTerminal: TIBDatabase;
    ...
    procedure setFDBRetaguarda(const Value: TIBDatabase);
    procedure setFDBTerminal(const Value: TIBDatabase);

    public
      constructor Create(AOwner: TComponent); override;
      destructor Destroy; override;
      procedure Execute;
      property DBRetaguarda : TIBDatabase read FDBRetaguarda write setFDBRetaguarda;
      property DBTerminal : TIBDatabase read FDBTerminal write setFDBTerminal;
  end;
...
begin
  ...
  FListaSincronia.DataBase := Self.FDBTerminal;
  // R = Receber. Checkpoint em TB_LISTA_SINCRONIA.LAST_UPDATE (decisoes 1 e 3);
  // kind '' = todas as linhas da tabela nesse sentido.
  LcTime := FListaSincronia.GetLastUpdate('R', Tabela, '');
  //Banco Servidor
...
begin
  ...
  Begin
    ...
    //set no local
    FSincronia.DataBase := Self.FDBTerminal;
    //Verifica se o CNPJ j� n�o existe
    LcLocal.Registro.CpfCNPJ := LcEmpresa.Registro.CpfCNPJ;
    LcLocal.getByDocumento;
    //Tratar somente se o cadastro n�o existir no terminal
...
begin
  // +1 segundo evita reprocessar o mesmo registro no próximo ciclo (regra já
  // existente). Checkpoint em TB_LISTA_SINCRONIA.LAST_UPDATE (decisoes 1 e 3).
  FListaSincronia.DataBase := Self.FDBTerminal;
  FListaSincronia.SetLastUpdate('R', Tabela, '', tempo + StrToTime('00:00:01'));
end;
