...
uses IBX.IBDatabase,Classes, Vcl.Grids,IBX.IBQuery, SysUtils,ControllerBase,
      Un_sistema,Un_funcoes, tblFinanceiro ,ObjFinancial,
...
begin
  ...
  Begin
    ...
    FormaPagto.getById;
    Obj.DescFormaPagamento := FormaPagto.Registro.Descricao;

    // Codigo/PlanoContas/Numero/Tipo/Situacao/Operacao/Fase removidos:
    // TFinancial (destiny/model) redesenhou tb_financial com PK NATURAL
    // (tb_order_id+tb_institution_id+terminal+parcel) — "semântica de
    // espelho" da revisão do sincronizador (financial_send_web.pas já
    // reescrito nesse formato; FIN_CODIGO não vira mais id, e os campos de
    // plano de contas/número/tipo/situação/operação/fase do título antigo
    // não existem no novo tb_financial). Este método (FillDataObjeto) não é
    // mais chamado pelo fluxo de envio atual — mantido só para compilar.
