...
uses
   ...
   IBUpdateSQL, IBSQL,forms,dialogs, windows,Graphics,ExtCtrls,
   jpeg, pngimage, ControllerPedido;
...
procedure TTrata_Excecao.reconectaBancos;
Begin
  {
  if GbSendToSrvLAN or GbReceiveFromSrvLAN then
  Begin
    try
      ConectaBDServerRec(True);
    except
      ConectaBDServerRec(False);
    end;

    try
      ConectaBDTerminal(True);
    except
      ConectaBDTerminal(False);
    end;
  End;

  if GbSendToSrvWAN then
  Begin
    try
      ConectaBDTerminal(True);
    except
